...
// @version      2.0
// @description  플라자CC 골프장 정기예약 자동화
...
(function(){
if(!/serviceS01/.test(window.location.href))return;
...
function filterAndSort(slots,s){
  ...
  var pri=s.course;
  if(pri==='T-OUT-first'){
    filtered.sort(function(a,b){var order={'T-OUT':0,'T-IN':1,'L-OUT':2,'L-IN':3};return(order[a.course]||9)-(order[b.course]||9)});
  }else if(pri==='T-IN-first'){
    filtered.sort(function(a,b){var order={'T-IN':0,'T-OUT':1,'L-IN':2,'L-OUT':3};return(order[a.course]||9)-(order[b.course]||9)});
...
panel.innerHTML=''+
'<div style="background:#2d6a4f;color:#fff;padding:10px 14px;border-radius:9px 9px 0 0;font-size:15px;font-weight:bold;cursor:move" id="m-header">플라자CC 예약 매크로 v2</div>'+
